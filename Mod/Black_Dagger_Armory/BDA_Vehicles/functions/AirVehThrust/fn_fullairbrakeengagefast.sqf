...
_pelican setVariable ["BDA_ThrustMode", 0, true];
...
while {((_pelican getVariable ["BDA_ThrustMode", 0]) == 0) AND (!(_pelican getvariable ["OPTRE_Thruster_EngagedStatus",false])) AND (speed _pelican > 90)} do
{
	_vel = velocity _pelican;
	_dir = direction _pelican;
	_speed = -14;
